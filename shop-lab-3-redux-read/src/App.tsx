...
class App extends Component<{}, State> {
  ...
  filter = () => {
    this.setState(previous => {
      ...
      if (!categoryToFilter)
        ...
        return { listedProducts: previous.allProducts.filter(item => item.category && item.category.includes(categoryToFilter)) }
    })
  }
  ...
  render() {
    return (
      <>
        <Menu products={[{name:'Product', price:123}]} title={'This is the main menu'} isLoading={this.state.isLoading} filter={this.state.filterName} items={this.menuItems} onClick={this.onFilter}></Menu>
        <div className="App">
